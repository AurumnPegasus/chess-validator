...
The bundled Python script installs `python-chess` into a local `.deps/` cache only when the package is not already importable. Review `requirements.txt` and the script before installing from an untrusted fork.
