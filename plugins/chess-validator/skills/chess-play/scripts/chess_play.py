#!/usr/bin/env python3
"""JSON chess CLI: render, validate, save, and apply legal moves."""
...
def ensure_chess() -> None:
    if chess is None:
        raise RuntimeError("Missing dependency: install with `python3 -m pip install 'chess>=1.11,<2'`.")
...
def save_slot(directory: Path, raw_name) -> tuple[Path, str]:
    ...
    return Path(directory) / f"{quote(name, safe='._-')}.json", name
...
def move_or_validate(cmd: str, board, moves, state: Path, move: str, by="human"):
    chosen, reason = None, "illegal"
    try:
        chosen = parse_move(board, move)
    except ValueError as exc:
        reason = str(exc)

    if cmd == "validate":
        ok = chosen is not None and chosen in board.legal_moves
        out = {"legal": ok, "move": move, "status": status(board, moves, not ok)}
        if ok:
    ...
    board.push(chosen)
    moves.append({"by": by, "san": san, "uci": chosen.uci()})
    save_state(state, board, moves)
...
def run(cmd: str, board, moves, state: Path, **args):
    ...
    if cmd == "prompt":
        return prompt(board), board, moves
    if cmd == "save-game":
    ...
    if cmd == "load-game":
        ...
        return out, loaded_board if loaded_board is not None else board, loaded_moves if loaded_moves is not None else moves
    if cmd == "list-saves":
        return list_saves(args["save_dir"]), board, moves
    if cmd in {"move", "validate"}:
        return move_or_validate(cmd, board, moves, state, args["move"], args.get("by", "human"))
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("status", "show-board"):
        sub.add_parser(name).add_argument("--legal", action="store_true")
    sub.add_parser("prompt")
    for name in ("save-game", "load-game"):
        sub.add_parser(name).add_argument("name", nargs="*", default=["default"])
    sub.add_parser("list-saves")
    for name in ("legal", "legal-moves"):
        sub.add_parser(name).add_argument("--square")
    sub.add_parser("validate").add_argument("move")
...
def main(argv=None) -> int:
    argv = [arg for arg in (sys.argv[1:] if argv is None else argv) if arg != "--json"]
    args = parser().parse_args(argv)
    try:
        ensure_chess()
        state, saves = path(args.state), save_dir(args.save_dir)
        if args.cmd == "serve":
            return serve(state, saves)
        if args.cmd in {"new", "new-game"}:
            if state.exists() and not args.force:
                return fail(f"{state} exists; use --force")
            out, _, _ = run(args.cmd, None, None, state)
        else:
            board, moves = load_state(state)
            opts = vars(args) | {"save_dir": saves}
            for key in ("cmd", "state"):
                opts.pop(key, None)
            out, _, _ = run(args.cmd, board, moves, state, **opts)
        emit(out)
        return 0
    except Exception as exc:
        return fail(str(exc))
