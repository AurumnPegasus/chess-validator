---
name: chess-play
description: Play against Claude or referee turn-by-turn chess directly inside Claude Code. Use when the user wants to start a chess game, play chess against Claude/the agent/LLM, render a chess board, validate whether a move is legal, apply a human or agent chess move, list legal moves, continue a saved chess position, or prevent LLM chess hallucinations such as illegal moves, lost board state, or invented pieces.
allowed-tools: Bash(${CLAUDE_SKILL_DIR}/scripts/chess_play.py *) Bash(python3 ${CLAUDE_SKILL_DIR}/scripts/chess_play.py *)
---
...
The script stores game state in `.claude-chess-game.json` in the current working directory by default. Use `--state <path>` for parallel games. It accepts SAN (`e4`, `Nf3`, `O-O`, `Qxf7#`) and UCI (`e2e4`, `g1f3`, `e7e8q`) moves.
...
In Claude Code, use one-shot JSON commands by default because Bash calls are not an interactive stdin channel:

Start or reset a game:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" new --force --json
```

Show the current board:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" status --json
```

Validate without changing state:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" validate "Nf3" --json
```

Apply a human move:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" move "e4" --by human --json
```

Build the minimal prompt for an LLM opponent:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" prompt --json
```

Apply Claude's selected move:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" move "c5" --by agent --json
```

List legal moves:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" legal --json
```

Let the script choose and apply a simple legal agent move:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" agent-move --json
```

If an interactive stdin-capable shell is available, one persistent process can avoid repeated Python imports:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/chess_play.py" serve
```
...
- Default to **opponent mode** when the user says "play chess", "let's play", "I play White/Black", or sends chess moves after starting a game. In opponent mode, Claude plays the other side.
- Use **referee-only mode** only when the user explicitly asks to validate moves, render a board, analyze a position, or manage a two-human/agent-vs-agent game without Claude playing.
...
- If `accepted` is `true` in opponent mode and the game is not over, immediately choose and apply Claude's reply before responding to the user. Do not stop at "Black/White to move."
- To choose Claude's reply, run `prompt --json`, choose exactly one UCI from the returned `legal_moves` using lightweight chess judgment, then run `move <uci> --by agent --json`.
